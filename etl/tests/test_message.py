...
def test_valid_delivery_time(test_archive, archive_msg, email_message):
...
def test_sent_date__make_aware_fails(test_archive, archive_msg, email_message):
...
def test_headers_clean(test_archive, archive_msg, email_message):
...
def test_headers_good_with_malformed_data(
    test_archive, archive_msg, email_message_w_bad_header
):
    """Make sure that transport headers don't blow up during import with a bad header"""
    with mock.patch(
        "etl.message.forms.p.parsestr", return_value=email_message_w_bad_header
    ):
        form = ArchiveMessageForm(archive=test_archive, archive_msg=archive_msg)
    assert form.is_valid()
    assert "headers" in form.cleaned_data
    assert "from" in form.cleaned_data["headers"]
    assert "Bad Header No Colon".lower() in form.cleaned_data["headers"]
